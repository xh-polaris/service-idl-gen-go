...
var (
	dataApiServiceInfo                = NewServiceInfo()
	dataApiServiceInfoForClient       = NewServiceInfoForClient()
	dataApiServiceInfoForStreamClient = NewServiceInfoForStreamClient()
)

// for server
func serviceInfo() *kitex.ServiceInfo {
	return dataApiServiceInfo
}

// for stream client
func serviceInfoForStreamClient() *kitex.ServiceInfo {
	return dataApiServiceInfoForStreamClient
}

// for client
func serviceInfoForClient() *kitex.ServiceInfo {
	return dataApiServiceInfoForClient
}
...
func reportEventHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(data.InsertRequest)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(data.DataApi).ReportEvent(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *ReportEventArgs:
		success, err := handler.(data.DataApi).ReportEvent(ctx, s.Req)
		if err != nil {
			return err
		}
...
var ReportEventArgs_Req_DEFAULT *data.InsertRequest

func (p *ReportEventArgs) GetReq() *data.InsertRequest {
	if !p.IsSetReq() {
...
type ReportEventResult struct {
	Success *data.InsertResponse
}

var ReportEventResult_Success_DEFAULT *data.InsertResponse

func (p *ReportEventResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(data.InsertResponse)
	}
...
func (p *ReportEventResult) GetSuccess() *data.InsertResponse {
	if !p.IsSetSuccess() {
...
func (p *ReportEventResult) SetSuccess(x interface{}) {
	p.Success = x.(*data.InsertResponse)
}
...
func (p *kClient) ReportEvent(ctx context.Context, Req *data.InsertRequest) (r *data.InsertResponse, err error) {
